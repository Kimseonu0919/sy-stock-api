...
## 🏛 Supported Brokers (지원 증권사)

현재 `sy-stock-api`는 한국투자증권(KIS)을 우선적으로 지원하며, 향후 다양한 증권사로 인터페이스를 확장할 예정입니다.

| 증권사 | 지원 여부 | 지원 기능 | 비고 |
| --- | --- | --- | --- |
| **한국투자증권 (KIS)** | ✅ 지원 | 국내 주식 시세/주문, 잔고 조회, 토큰 캐싱 | REST API 기반 |
| **키움증권 (Kiwoom)** | ⏳ 예정 | - | Open API 기반 지원 예정 |
| **KB증권 (M-able)** | ⏳ 예정 | - | - |
